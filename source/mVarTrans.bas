...
' ----------------------------------------------------------------------------
' Writes all items of an array (a_arry) to a file (a_file) - which might be
' a file object, a file's full name or not provided - as records/lines. When
' no file (a_file) is provided a temporary file is written and returned.
' ----------------------------------------------------------------------------
...
Public Function ArrayAsString(ByVal a_array As Variant) As String
' ----------------------------------------------------------------------------
' Returns an array (a_array) as string with the items delimited by a vbCrLf.
' ----------------------------------------------------------------------------
    Dim s As String
...
Public Function FileAsArray(ByVal f_file As Variant) As Variant
' ----------------------------------------------------------------------------
...
Public Function FileAsStringEmptyExcluded(ByVal f_s As String) As String
' ----------------------------------------------------------------------------
...
' ----------------------------------------------------------------------------
    ...
    Select Case TypeName(v_items)
        ...
        Case "File":
            Select Case v_items_as
                Case enAsArray:        VarTrans = FileAsArray(v_items)
        ...
        Case Else
            ...
            Else
                Select Case v_items_as
                    ...
                    Case enAsString:       VarTrans = StringAsString(CStr(v_items))
                End Select
            End If
    End Select
